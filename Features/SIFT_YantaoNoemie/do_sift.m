...
for o=1:scalespace.O
	...
	fprintf('CS5240 -- SIFT: computing octave %d\n', o-1+omin) ;
                tic ;
    %size(
	%[r,c,max_local] = findLocalMaximum(difofg.octave{o},40.0);
...
%		size(oframes)
  % Store frames
	...
	%frames = [frames, [x(:)' ; y(:)' ; sigma(:)' ; oframes(4,:);val(:)'] ] ;
    
    frames = [frames,[oframes(1,:);oframes(2,:);oframes(3,:);val(:)']];

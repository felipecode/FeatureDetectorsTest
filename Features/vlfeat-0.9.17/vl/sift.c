 ...
 ** ::vl_iisift_get_keypoints(). This function just use the HVS instead of the regular sift
 **
 ** @param f SIFT filter.
 **/
...
vl_iisift_detect (VlSiftFilt * f)
{
  ...
  pt = f-> dog ;
  memmove( (void*)ptii, (void*)pt,  sizeof(vl_sift_pix) );
  ...
  for (s = s_min ; s <= s_max - 1 ; ++s) {
    ...
    while (src_a != end_a) {
      *pt++ = *src_b++ - *src_a++ ;
      vl_sift_pix aux;
       aux= (*src_b++ - *src_a++)/(*src_b++ + *src_a++) ; // Maybe we need to check for some overlaps
       if ( *src_b < *src_a){
           if(*pt < aux)
               *ptii++ = *pt;
           else

...
void compute_inliers_ransac(const std::vector<cv::Point2f> &matches, std::vector<cv::Point2f> &inliers,
  ...
  if (use_fund == true) {
    H = findFundamentalMat(points1,points2,FM_RANSAC,error,0.99,status);
  }
  else {
    H = findHomography(points1,points2,RANSAC,error,status);
  }
...
void draw_inliers(const cv::Mat &img1, const cv::Mat &img2, cv::Mat &img_com,
  ...
  resize(img2,img_aux,Size(img1.cols,img1.rows),0,0,INTER_LINEAR);

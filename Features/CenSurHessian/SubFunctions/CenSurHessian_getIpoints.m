function ipts=CenSurHessian_getIpoints(FastHessianData,verbose)
% filter index map
...
responseMap=CenSurHessian_buildResponseMap(FastHessianData);
...
for o = 1:FastHessianData.octaves
    for i = 1:2
        ...
        r=r(:); c=c(:);
        p =find(CenSurHessian_isExtremum(r, c, t, m, b,FastHessianData));
%         ipts= [ipts ; [ c(p) r(p)]];
%         
%         np = np + size(p,1);
        ...
        for j=1:length(p);
           ind=p(j);
           [ipts,np]=CenSurHessian_interpolateExtremum(r(ind), c(ind), t, m, b, ipts,np);
        end
    end
end

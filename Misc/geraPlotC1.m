...
% 
    ...
    pt = plot(x,harvec,'b-d',x,hesvec,'r-*',x,lapvec,'m-o',x,harcolor,'g-s',x,harcolorb,'g-.x',x,harlapvec,'k-s',x,heslapvec,'c-o',x,siftvec,'m-x',x,surfvec,'c-.*',x,kaze,'b-*');
    %set(gca,'XScale','log'); 
    set(gca, 'XGrid','on');
    set(gca, 'YGrid','on');
    legend1=legend('Harris','Hessian','Laplacian','HarCol','HarColB','HarLap','HesLap','DoG','FastHes','kaze');
    ...
    ylim([0 max([max(harvec),max(hesvec),max(lapvec),max(harcolor),max(harcolorb),max(harlapvec),max(heslapvec),max(siftvec),max(surfvec),max(kaze)])]);
    xlim([0 max(x)]);
    for i=1:11
        xtl{i} = {juice(i);x(i)};
    end

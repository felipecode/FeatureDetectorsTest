function idt = calculateError(J,I,name)

if (size(J) ~= size(I))
    disp('The size of the 2 matrix are unequal')
    
    idt = NaN;
    return;
elseif (I == J)
    disp('Images are identical: idt zero value')
    ...
    if strcmp(name,'MSE')
        ...
        [M N] = size(J);
        error = I - J;
        idt = sum(sum(error .* error)) / (M * N);
        
        
    elseif strcmp(name,'PSNR')
        
        
        maxValue = double(max(J(:)));
        
        % Calculate MSE, mean square error.
        mseImage = (double(J) - double(I)) .^ 2;
        [rows columns] = size(J);
        
        mse = sum(mseImage(:)) / (rows * columns);
        
        % Calculate PSNR (Peak Signal to noise ratio)
        idt = 10 * log10( 256^2 / mse);

    elseif strcmp(name,'AVG')
        
        
        idt=sum(sum(I))/(size(I,1)*size(I,2));
